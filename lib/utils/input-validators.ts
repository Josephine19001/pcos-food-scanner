...
export function isSalutation(message: string): boolean {
  ...
  return salutations.some(greeting => 
...
export function isValidName(message: string): boolean {
  ...
  return namePattern.test(trimmed) && 
         ...
         !/^\s+$/.test(trimmed); // Not just spaces
}

export function isDateFormat(message: string): boolean {
  console.log('🔍 DATE VALIDATION START');
  ...
  const patternResults = datePatterns.map((pattern, index) => {
    ...
    console.log(`Pattern ${index}: ${result} - ${pattern.toString()}`);
    return result;
  });
  
  const passesRegex = patternResults.some(result => result);
  console.log('Passes regex:', passesRegex);
  
  if (!passesRegex) {
    console.log('❌ FAILED at regex stage');
    return false;
  }
  ...
  try {
    ...
    if (isNaN(date.getTime())) {
      ...
      console.log('Trying without comma:', JSON.stringify(withoutComma));
      date = new Date(withoutComma);
      console.log('Second parse attempt:', date);
      ...
      if (isNaN(date.getTime())) {
        ...
        for (let i = 0; i < monthNames.length; i++) {
          if (lowerNormalized.includes(monthNames[i]) || lowerNormalized.includes(shortMonthNames[i])) {
            monthIndex = i;
            break;
          }
        }
        ...
        if (monthIndex >= 0) {
          ...
          if (dayYearMatch) {
            ...
            console.log('Trying Date constructor with numbers:', year, month, day);
            date = new Date(year, month, day);
            console.log('Third parse attempt:', date);
            
            // Also try ISO format as backup
            if (isNaN(date.getTime())) {
              const isoFormat = `${year}-${String(month + 1).padStart(2, '0')}-${String(day).padStart(2, '0')}`;
              console.log('Trying ISO format:', isoFormat);
              date = new Date(isoFormat);
              console.log('Fourth parse attempt:', date);
            }
          }
        }
      }
    ...
    console.log('Date >= hundred years ago:', date >= hundredYearsAgo);
    console.log('Now:', now);
    console.log('Hundred years ago:', hundredYearsAgo);
