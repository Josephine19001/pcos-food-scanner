    ...
    if (visible) {
      if (!initializedRef.current) {
        if (initialMessages.length > 0) {
          // Use initial messages (restored from storage)
          ...
          console.log('Loading intro messages:', introMessages.length);
        ...
        // Handle case where initial messages come in after initialization
        console.log('Updating with late initial messages:', initialMessages.length);
        setMessages(initialMessages);
      }
    } else if (!visible) {
    ...
    >
      <View className={themed('flex-1 bg-white', 'flex-1 bg-gray-900')}>
        <Animated.View
          ...
          exiting={context === 'onboarding' ? SlideOutLeft.duration(300) : SlideOutRight.duration(300)}
          style={{ flex: 1 }}
        >
        <KeyboardAvoidingView
          style={{ flex: 1 }}
            ...
            <View className={themed('flex-1 bg-white', 'flex-1 bg-gray-900')}>
              ...
              >
                {questionId === 'complete' ? (
                  ...
                  />
                ) : (questionId === 'body-height' || questionId === 'body-weight' || questionId === 'body-goal-weight-input') ? (
                  <NumberInput
                    value={inputText}
                    onChangeText={setInputText}
                    placeholder={
