  ...
  return (
    <View className="mb-4">
      ...
      >
        <Text
          className={themed(
            `text-base ${selectedValues.length > 0 ? 'text-black font-medium' : 'text-gray-400'}`,
            `text-base ${selectedValues.length > 0 ? 'text-white font-medium' : 'text-gray-400'}`
          )}
          numberOfLines={1}
        >
          {getDisplayText()}
      ...
      {error && (
        <Text className={themed('text-red-500 text-sm mt-1', 'text-red-400 text-sm mt-1')}>
          {error}
        </Text>
      )}
          ...
          >
            <View className={themed('p-6 border-b border-gray-50', 'p-6 border-b ')}>
              ...
              {multiSelect && (
                <Text
                  className={themed(
                    'text-sm text-pink-500 mt-2 font-medium',
                    'text-sm text-pink-400 mt-2 font-medium'
                  )}
                >
            ...
            <FlatList
              ...
              renderItem={({ item }) => (
                <TouchableOpacity
                  ...
                  `,
                    `
                    p-5 border-b  flex-row items-center justify-between
                ...
                >
                  ...
                  {isSelected(item.value) && (
                    <Check size={20} color={colors.isDark ? '#F472B6' : '#EC4899'} />
                  )}
                </TouchableOpacity>
              )}
            />
